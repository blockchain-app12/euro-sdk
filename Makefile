...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/osiz-blockchainapp/euro-sdk/types"
	godoc -http=:6060
...
test_ledger_mock:
	@go test -mod=readonly `go list github.com/osiz-blockchainapp/euro-sdk/crypto` -tags='cgo ledger test_ledger_mock'

test_ledger: test_ledger_mock
	@go test -mod=readonly -v `go list github.com/osiz-blockchainapp/euro-sdk/crypto` -tags='cgo ledger'
...
format: tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs goimports -w -local github.com/osiz-blockchainapp/euro-sdk
.PHONY: format
...
devdoc_init:
	docker run -it -v "$(CURDIR):/go/src/github.com/osiz-blockchainapp/euro-sdk" -w "/go/src/github.com/osiz-blockchainapp/euro-sdk" tendermint/devdoc echo
	# TODO make this safer
	$(call DEVDOC_SAVE)

devdoc:
	docker run -it -v "$(CURDIR):/go/src/github.com/osiz-blockchainapp/euro-sdk" -w "/go/src/github.com/osiz-blockchainapp/euro-sdk" devdoc:local bash
